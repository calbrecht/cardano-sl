...
createCommandProcs hasAuxxMode printAction mSendActions = rights . fix $ \commands -> [
    ...
    let name = "send-without-witness" in
    needsSendActions name >>= \sendActions ->
    needsAuxxMode name >>= \Dict ->
    return CommandProc
    { cpName = name
    , cpArgumentPrepare = identity
    , cpArgumentConsumer = getArg tyTxOut "out"
    , cpExec = \output -> do
        Tx.sendWithoutWitnesses sendActions output
        return ValueUnit
    , cpHelp = "send a transaction with fake input without witnesses  \
               \ (for testing purposes only)"
    },
